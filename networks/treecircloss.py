...
class TreeCIRLoss(torch.nn.Module):
    def __init__(self, args):
        super(TreeCIRLoss, self).__init__()
        self.B = args.B
        self.m = args.m
    ...
    def forward(self, mat, pos_mask, neg_mask, rd):
        ...
        for i in range(self.B):
            neg_rd = torch.exp(1-rd[i])
            pos_rd = torch.exp(rd[i])
            neg_reshape = eq_sn[i].repeat(N,1)
            pos_reshape = eq_sp[i].repeat(M,1).t()
            expsum = torch.sum(torch.exp(neg_rd * neg_reshape + pos_rd * pos_reshape))
